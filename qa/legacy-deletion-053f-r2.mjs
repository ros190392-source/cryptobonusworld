...
const redirects = [
  ...['bybit','mexc','okx','bitget','kucoin','bingx','coinex'].map(slug => ({
    path: `/exchanges/${slug}/`,
    target: `/${slug}/`,
    ...
    for (const viewport of viewports) {
      ...
      if (route.family === 'homepage') {
        Object.assign(checks, {
          ...
          rankingBeginsFirstViewport: await page.locator('.top10-section').evaluate((node, height) => node.getBoundingClientRect().top < height, viewport.height),
        });
      }
      ...
      if (route.family === 'faq') {
        ...
        Object.assign(checks, {
          ...
          faqSchemaParity: Array.isArray(faqSchema?.mainEntity) && faqSchema.mainEntity.length === 20,
...
await writeFile(`${outDir}/CBW_LEGACY_DELETION_053F_R2.json`, JSON.stringify(report, null, 2));
