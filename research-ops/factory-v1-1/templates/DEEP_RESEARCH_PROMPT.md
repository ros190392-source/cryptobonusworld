# Deep Research start prompt — {{EXCHANGE_NAME}} × {{COUNTRY_NAME}}

TASK: `{{TASK_ID}}`
PROJECT: CryptoBonusWorld
COUNTRY: {{COUNTRY_NAME}} (`{{COUNTRY_CODE}}`)
EXCHANGE: {{EXCHANGE_NAME}} (`{{EXCHANGE_ID}}`)
BATCH: `{{BATCH_ID}}` · PRIORITY: `{{PRIORITY}}`
FACTORY: ResearchOps Subscription Factory v{{FACTORY_VERSION}}
...
Research languages: English, Russian, and the primary local language of {{COUNTRY_NAME}} where official sources exist.
...
a regulator warning alone. Do not use EEA/MiCA or other foreign licensing as {{COUNTRY_NAME}} evidence.
