...
const CONTAINER_SELECTORS = [
  ...
  '.info-page .container', '.container', '.page-hero__inner', '.audit-wrap',
];
...
async function firstVisibleRect(page, selector) {
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    if (await locator.isVisible().catch(() => false)) return rectOrNull(await locator.boundingBox());
  }
  return null;
}
...
function evaluateFirstScreen(route, viewport, { h1Rect, heroRect, usefulRect, fullRankingRows, rankingRowsBegun, ctaVisible }) {
  if (route.contract === 'homepage') {
    if (viewport.width >= 1000) return Boolean(usefulRect && usefulRect.top < viewport.height && fullRankingRows >= 3);
    if (viewport.width <= 480) return Boolean(usefulRect && usefulRect.top < viewport.height && rankingRowsBegun >= 1);
    return Boolean(usefulRect && usefulRect.top < viewport.height);
  }
  if (route.contract === 'exchange') {
    return Boolean(heroRect && usefulRect && heroRect.top < viewport.height && usefulRect.top < viewport.height && ctaVisible);
  }
  return Boolean(h1Rect && h1Rect.bottom < viewport.height && usefulRect && usefulRect.top < viewport.height);
}

export async function measureFirstViewports(options = {}) {
  const baseUrl = options.baseUrl ?? 'http://127.0.0.1:4321';
  const outDir = path.resolve(options.outDir ?? path.join(process.cwd(), 'reports', 'site-standard'));
  ...
  try {
    for (const route of SITE_STANDARD_ROUTES) {
      for (const viewport of SITE_STANDARD_VIEWPORTS) {
        ...
        const firstScreenPass = evaluateFirstScreen(route, viewport, {
          h1Rect,
          heroRect,
          usefulRect,
          fullRankingRows,
  ...
  await fs.writeFile(path.join(outDir, 'CBW_FIRST_VIEWPORT_MEASUREMENTS_053A.json'), `${JSON.stringify(report, null, 2)}\n`);
  const rows = results.map(item => `| ${item.route.path} | ${item.route.family} | ${item.viewport.name} | ${item.firstScreenPass ? 'PASS' : 'FAIL'} | ${item.rects.firstUseful?.top ?? 'missing'} | ${item.widestContainer?.width ?? 'n/a'} | ${item.fullRankingRows} | ${item.horizontalOverflow} |`);
  const markdown = [
    ...
    `- First-screen passes: ${summary.firstScreenPasses}`,
    `- First-screen failures: ${summary.firstScreenFailures}`,
    `- Overflow failures: ${summary.overflowFailures}`,
    `- Measurements with unexpected errors: ${summary.unexpectedErrorMeasurements}`,
    '',
    '## Homepage',
    '',
    ...summary.homepage.map(item => `- ${item.viewport}: ${item.pass ? 'PASS' : 'FAIL'}; rankingTop=${item.rankingTop}; fullRows=${item.fullRows}; rowsBegun=${item.rowsBegun}; blankBeforeRanking=${item.blankBeforeRanking}`),
    '',
    '## All measurements',
    '',
  ...
  await fs.writeFile(path.join(outDir, 'CBW_FIRST_VIEWPORT_MEASUREMENTS_053A.md'), markdown);
  return report;
}

if (process.argv[1] && path.resolve(process.argv[1]) === path.resolve(new URL(import.meta.url).pathname)) {
  measureFirstViewports().catch(error => {
    console.error(error);
    process.exitCode = 1;
  });
}
